...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public ApiResponse<EmployeeResponse> createEmployee(EmployeeRequest request) {
		...
		return new ApiResponse<>("SUCCESS", "Employee created successfully", response);
	}
	...
	public ApiResponse<List<EmployeeResponse>> getAllEmployees() {
		...
		List<EmployeeResponse> responseList = employees.stream().map(employeeMapper::mapToDto)
				.collect(Collectors.toList());

		return new ApiResponse<>("SUCCESS", "Employee list fetched successfully", responseList);
	}
	...
	public ApiResponse<List<EmployeeResponse>> searchEmployees(String search) {
		...
		if (employees.isEmpty()) {
			throw new BaseException("EMP_404", "No employee found for search keyword : " + search,
		...
		return new ApiResponse<>("SUCCESS", "Employee search result", responseList);
	}
}
